; change Turner.ahk to Turner.exe before compiling
...
gui, main:add, text, ys+4, How many wins will the bot go for?

gui, main:add, DropDownList, section xs+1 vending_ddl gsubmit_ddl AltSubmit, Pause bot|Close bot|Close bot & MTGA|Turn off PC
...
if (FolderText = "")
	{
		GuiControl, main:text, filetext, Please select the MtgaLauncher exe file
	}
else
	{
...
gui, main:show, autosize, Sparkless
...
FormatresetT(ms)
	{
		sec := SubStr(ms, 1, -3)
		;~ min := Floor(sec/60) ------------------------------------------------------------------------ommited because it would reset secs after 60-------------------------------------------------------------------
		;~ sec := sec-(min*60)
		;~ hrs := Floor(min/60)
		;~ min := min-(hrs*60)
		;~ While StrLen(min) = 0
...
reset:
	...
	If ErrorLevel
		{
			if(resettime>=600) ;after 600 seconds of not detecting either my turn or my opponents turn, it closes down MTGA and reopens it
				{
...
gamestate() ;checks state of screen for various indicators that the match is over/needs to begin.
		...
		If (ErrorLevel = 0)
			{
				...
				MouseClick
				Sleep 1000
				MouseMove % stateposX -200, %stateposY%,10
			}
		...
		If (ErrorLevel = 0)
			{
				...
				PostMessage, 9000,,,,turnloops
				MouseMove %stateposX%,% stateposY + 100
				MouseClick
			}
	...
	WinActivate, MTGA
	ImageSearch, stateposX, stateposY, 590, 250, 670, 350, *60 *TransBlack %A_WorkingDir%\images\fun.png
		If (ErrorLevel == 0)
			{
				MouseMove 635, 563, 10
				MouseClick
				Sleep 10
				MouseClick
				Sleep 1000
				MouseMove 350,150, 10
			}
